...
public:
  ...
  : Node("color_grasp_moveit", options)
  {
    ...
    prepare_approach_height_m_ = declare_parameter<double>("prepare_approach_height_m", 0.10);
    ...
    prepare_service_ = create_service<std_srvs::srv::Trigger>(
      ...
      [this](
        const std::shared_ptr<std_srvs::srv::Trigger::Request>,
        std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
        response->success = prepareGraspPose();
        response->message = response->success ? "prepare grasp pose finished" : "prepare grasp pose failed";
      });
...
private:
  ...
  bool prepareGraspPose()
  {
    geometry_msgs::msg::PoseStamped target;
    {
      std::lock_guard<std::mutex> lock(target_mutex_);
      if (!have_target_) {
        RCLCPP_WARN(get_logger(), "prepareGraspPose failed: no target pose received yet.");
        return false;
      }
      target = latest_target_;
    ...
    if (!isTargetInsideWorkspace(target)) {
      RCLCPP_WARN(
        get_logger(),
        "prepareGraspPose rejected target outside workspace: x=%.3f y=%.3f z=%.3f",
        target.pose.position.x, target.pose.position.y, target.pose.position.z);
      return false;
    }

    // Optionally open gripper on prepare; default is false (keep closed while approaching)
    const auto open_on_prepare = get_parameter_or<bool>("open_gripper_on_prepare", false);
    const auto open_target = get_parameter_or<std::string>("open_gripper_target", "open");
    if (open_on_prepare) {
      if (!moveGripper(open_target)) {
        RCLCPP_WARN(get_logger(), "prepareGraspPose failed: opening gripper failed.");
        return false;
      }
    }

    geometry_msgs::msg::PoseStamped pregrasp = target;
    pregrasp.pose.position.z += prepare_approach_height_m_;

    if (!moveArm(pregrasp, "prepare_grasp")) {
      RCLCPP_WARN(get_logger(), "prepareGraspPose failed: pregrasp motion failed.");
      return false;
    }

    RCLCPP_INFO(
      get_logger(),
      "prepareGraspPose succeeded: pregrasp pose [x=%.3f y=%.3f z=%.3f]",
  ...
  double prepare_approach_height_m_{0.10};
